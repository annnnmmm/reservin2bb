...
try:
    connection = psycopg2.connect(dbname="tables", user="postgres", password="1234", host="localhost")
    ...
    async def cancel_reserve(request: Request):
        try:
            datetime_obj = datetime.strptime(
                datetime.fromtimestamp(time.time(), pytz.timezone('Europe/Moscow')).strftime('%d/%m/%Y'), "%d/%m/%Y")
            date_today = int(datetime_obj.timestamp())
            with connection.cursor() as cursor:
                cursor.execute(
                    "select id, table_id, name, date_reserve from reservations where date_reserve >= %(date)s and user_id = %(user_id)s order by date_reserve desc",
                    {"date": date_today, "user_id": request.cookies.get('id')})
                get_my_reservation = cursor.fetchone()
                cursor.execute(
            ...
            print(ex)
            return JSONResponse(status_code=200, content={"result": "error", "text_error": str(ex)})

    @app.post("/reserve_table")
    async def reserve_table(request: Request, name: str = Form(...), date: str = Form(...),
                            count_guests: str = Form(...), time: str = Form(...)):
        try:
            combined_datetime = datetime.strptime(f"{date} {time}", "%d/%m/%y %H:%M")
            combined_datetime_obj = datetime.strptime(f"{date} {time}", "%d/%m/%y %H:%M")
            timestamp = int(combined_datetime_obj.timestamp())
            user_id = request.cookies.get('id')
            ...
            with connection.cursor() as cursor:
                cursor.execute(
                    ...
                    {"date": timestamp, "user_id": request.cookies.get('id')})
                get_my_reservation = cursor.fetchone()
                    ...
                    if get_table is not None:
                        ...
                        if table_reserve > 0:
                            ...
                            emailLib.sendEmail(mailserver, admin_email, "Забронирован столик",
                                               f"Был забронирован столик; Почта клиента: {user[0]}; Дата, на которую забронировано: {date}; Время, на которое забронировано: {time}; Количество гостей: {count_guests}; Номер забронированного столика: {table_reserve}")
